...
require($_SERVER["DOCUMENT_ROOT"] . "config.php");
  class func
  {
    ...
    require($_SERVER["DOCUMENT_ROOT"] . "config.php");
		...
		$abfrage = "SELECT * FROM `" . $config["dbprefix"] . "_Identifier` WHERE `Identifier` = '$identifier'";
		$res = $mysqli->query($abfrage);
			if ( $res->num_rows > 0){
			$row = $res->fetch_object();
			$getkey = $this->get_key($row->Key_ID);
			if (strtotime($getkey[0]["Life_Time"]) >= time()){
			return true;
			} else {
			return 300;
			} 
	...
	$identifier = str_shuffle("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789");	require($_SERVER["DOCUMENT_ROOT"] . "config.php");
			...
			$abfrage = "SELECT * FROM `" . $config["dbprefix"] . "_key` WHERE `key` = '$key' AND `Life_Time` >= '$ts';";
			$res = $mysqli->query($abfrage);
			if ( $res->num_rows > 0){
			$row = $res->fetch_object();
		...
		$abfrage = "SELECT * FROM `$table` WHERE `ID` = '$keyID'";
		}
		if ($res = $mysqli->query($abfrage)){
		$i = 0;
		...
		$conte[$i]["Life_Time"] = $row->Life_Time ;
		$i ++;
		}
		return $conte;
		} 
		...
		public function get_software($key)
		{
		require($_SERVER["DOCUMENT_ROOT"] . "config.php");
		$mysqli = $this->mysql_con();
...
if ($res = $mysqli->query($abfrage)){
		...
		public function stati_key_lcount($last_time)
		{
		require($_SERVER["DOCUMENT_ROOT"] . "config.php");
		$mysqli = $this->mysql_con();
		$table = $config["dbprefix"] . "_key";
		$abfrage = "SELECT * FROM `$table` WHERE `create_date` > '" . $last_time . "'";
		if ($res = $mysqli->query($abfrage)){
		$licount;
		while ($row = $res->fetch_object()) {
	 $licount = $licount + $row->licens_counter ;
		}
		return $licount;
		} 
		echo $mysqli->error;
		...
		public function stati_key_aktivations($last_time)
		{
		require($_SERVER["DOCUMENT_ROOT"] . "config.php");
		$mysqli = $this->mysql_con();
		$table = $config["dbprefix"] . "_key";
		...
		$licount;
		while ($row = $res->fetch_object()) {
	 $licount = $licount + $row->activations ;
		}
		return $licount;
		} 
		echo $mysqli->error;
